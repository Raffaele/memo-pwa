<script>
  ...
  function getCardMap(mapSize) {
    const sortedArray = Array.from({ length: mapSize }).map((_, i) => i);

    return shuffle(sortedArray);
  }

  function createCards(numOfCouples, numOfUnpair) {
    const completeCouples = Array.from({ length: numOfCouples })
      .map((_, i) => [i, i])
      .flat();
    const notRepeated = Array.from({ length: numOfUnpair }).map(
      (_, i) => i + numOfCouples
    );
    ...
    {#each cardLines as cardLine}
      <div class="game-line">
        {#each cardLine as singleCard}
          <div
            class="card status--{singleCard.status}"
            on:click={() => handleCardClick(singleCard)}
          >
